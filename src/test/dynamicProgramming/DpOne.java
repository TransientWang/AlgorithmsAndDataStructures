...
//            dp[i] = (s.charAt(i - 1) == '0') ? 0 : dp[i - 1];
    ...
    public void testTwo() {
        ...
        int[] ints2 = new int[]{0};
        int[] ints3 = new int[]{-2, 0, 1, 1, 2};
        Object[] objects = {ints, ints1};
        TestHelper.test(getClass(), "threeSumOne", ints1);
//        TestHelper.test(getClass(), "threeSum", ints);
    ...
    /**
     * @return java.util.List<java.util.Listjava.lang.Integer>>
     * @Description 三数之和
     * 这种方法超时
     * 太麻烦，不够优化
     * @Param [nums]
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        for (int i = 0; i < dp.length; i++) {
            for (int j = 1; j < dp.length; j++) {
                Integer tmp = 0;
                if (i < j) {
                    if ((tmp = map.get(-dp[i][j])) != null && (i < j && j < tmp)) {
        ...
        int ien = 0;
        while (ien < lists.size() - 1) {
            List<Integer> integers = lists.get(ien);
            List<Integer> integers1 = lists.get(ien + 1);
            if (integers.get(2) == integers1.get(2) && integers.get(0) == integers1.get(0) && integers.get(1) == integers1.get(1)) {
                lists.remove(ien);
            } else
                ien++;
